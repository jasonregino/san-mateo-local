...
function proximityBlock(anchor) {
  ...
  const lines = near.map(r => `- ${r.p.name} | ${r.p.cat} | ${r.p.type || r.p.cat} | ${r.mi.toFixed(2)} mi | ${(r.p.about || '').replace(/\s+/g, ' ').trim()}`).join('\n');
  return `NEARBY LIST (internal, from ${anchor.label}; each line is: name | category | type | REAL distance in miles | description, nearest first). This is the ONLY source of truth for anything about close, nearby, near me, walking distance, or "closest". Never mention this list, never say "proximity context", just speak naturally about how close things are. You ALREADY know where the visitor is (${anchor.label}); do NOT ask them for their neighborhood or nearest cross street again, just give them the closest options with confidence.

HOW TO ANSWER A NEARBY REQUEST:
1. Match what they asked for to BOTH the type AND the description, NOT just the name. The description is what a place actually offers: if it names the item, that place fits, even if the type does not (Aldo's Pizza "also serves sandwiches" fits a sandwich ask; Ay Caray's "tortas (Mexican sandwiches)" fits; a cleaner whose description says "alterations" does alterations). Also, a deli, market, or taqueria can do a sandwich, and a bakery or cafe has pastries, even if not spelled out. READ every nearby line's description before answering. Do not skip a closer place that genuinely fits to feature a farther one whose name matches better.
